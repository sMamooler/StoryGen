...
from dataset import StorySalonDataset, VWPDataset
...
class SampleLogger:
    ...
    def log_sample_images(
        ...
        for idx, prompt in enumerate(
            tqdm(self.prompts, desc="Generating sample images")
        ):
            ...
            image = (image + 1.0) / 2.0  # for visualization
            image = image.squeeze().permute(1, 2, 0).detach().cpu().numpy()
            cv2.imwrite(
                os.path.join(self.logdir, f"{step}_{idx}_{seed}.png"),
                image[:, :, ::-1] * 255,
            )
            v_refs = []
            ...
            for i in range(len(v_refs)):
                cv2.imwrite(
                    os.path.join(self.logdir, f"{step}_{idx}_{seed}_ref_{i}.png"),
                    v_refs[i][:, :, ::-1] * 255,
                )

            with open(
                os.path.join(self.logdir, f"{step}_{idx}_{seed}" + ".txt"), "a"
            ) as f:
            ...
            for i, img in enumerate(sequence):
                img[0].save(
                    os.path.join(
                        self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output.png"
                    )
                )
...
def train(
    ...
    fn_args = get_function_args()
    time_string = get_time_string()
    logdir = args.logdir + f"_{time_string}"
    ...
    if accelerator.is_main_process:
        ...
        OmegaConf.save(fn_args, os.path.join(logdir, "config.yml"))
    ...
    pretrained_sdm = torch.load(
        f"{args.pretrained_model_path}/unet/diffusion_pytorch_model.bin",
        map_location="cpu",
    )
    ...
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.val_batch_size, shuffle=False, num_workers=1
    )
    ...
    if args.validation_sample_logger is not None and accelerator.is_main_process:
        validation_sample_logger = SampleLogger(
            **args.validation_sample_logger, logdir=logdir
        )

    progress_bar = tqdm(
        range(step, args.train_steps), disable=not accelerator.is_local_main_process
    )
    ...
    while step < args.train_steps:
        ...
        mask = F.interpolate(
            mask, scale_factor=1 / 8.0, mode="bilinear", align_corners=False
        )
        b, c, h, w = image.shape
        ...
        timesteps = torch.randint(
            0, noise_scheduler.config.num_train_timesteps, (b,), device=latents.device
        )
        ...
        encoder_hidden_states = text_encoder(prompt_ids.to(accelerator.device))[
            0
        ]  # B * 77 * 768
        ...
        if accelerator.sync_gradients:
            ...
            if accelerator.is_main_process:
                if (
                    validation_sample_logger is not None
